...
unsigned long g_twl6040_reg = 0;
struct twl6040_codec *codec_twl6040;
...
static ssize_t twl6040_w_val(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char * endp = (char *)buf;
    unsigned long val = simple_strtoul(buf, &endp, 0);

	if((val > 0xff) || (endp == buf))
		return -EINVAL;

	twl6040_reg_write(codec_twl6040, g_twl6040_reg, (int)val);

    return count;
}

static DEVICE_ATTR(val, S_IWUSR | S_IRUGO, twl6040_r_val, twl6040_w_val);

static ssize_t twl6040_r_reg(struct device *dev, struct device_attribute *attr, char *buf)
{
    printk("0x");
    return sprintf(buf, "%.2x\n", (int)g_twl6040_reg);
}

static ssize_t twl6040_w_reg(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char * endp = (char *)buf;
    unsigned long val = simple_strtoul(buf, &endp, 0);
...
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, twl6040_r_reg, twl6040_w_reg);
...
static DEVICE_ATTR(prt, S_IWUSR, NULL, twl6040_prt);
